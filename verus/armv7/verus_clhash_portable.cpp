...
// verus intermediate hash extra - v2.1
__m128i __verusclmulwithoutreduction64alignedrepeat_v2_1(__m128i *randomsource, const __m128i buf[4], uint64_t keyMask, uint32_t * __restrict fixrand, uint32_t * __restrict fixrandex, u128 *g_prand, u128 *g_prandex)
{
  const __m128i pbuf_copy[4] = {_mm_xor_si128(buf[0],buf[2]), _mm_xor_si128(buf[1],buf[3]), buf[2], buf[3]};
    const  __m128i *pbuf;

    // v2.1/v2.2 do not shift keyMask
    // keyMask >>= 4;
...
// verus intermediate hash extra - v2.2 (same as v2.1 for portable implementation)
__m128i __verusclmulwithoutreduction64alignedrepeat_v2_2(__m128i *randomsource, const __m128i buf[4], uint64_t keyMask, uint32_t * __restrict fixrand, uint32_t * __restrict fixrandex, u128 *g_prand, u128 *g_prandex)
{
    // v2.2 is identical to v2.1 for the portable implementation
    return __verusclmulwithoutreduction64alignedrepeat_v2_1(randomsource, buf, keyMask, fixrand, fixrandex, g_prand, g_prandex);
}

// verus intermediate hash extra - OLD VERSION (for backward compatibility)
...
uint64_t verusclhashv2_1(void * random, const unsigned char buf[64], uint64_t keyMask, uint32_t *  __restrict fixrand, uint32_t * __restrict fixrandex, __m128i *g_prand, __m128i *g_prandex) {
    ...
    __m128i  acc = __verusclmulwithoutreduction64alignedrepeat_v2_1(rs64, string, keyMask, fixrand, fixrandex, g_prand, g_prandex);
...
uint64_t verusclhashv2_2(void * random, const unsigned char buf[64], uint64_t keyMask, uint32_t *  __restrict fixrand, uint32_t * __restrict fixrandex, __m128i *g_prand, __m128i *g_prandex) {
    ...
    __m128i  acc = __verusclmulwithoutreduction64alignedrepeat_v2_2(rs64, string, keyMask, fixrand, fixrandex, g_prand, g_prandex);
    acc = _mm_xor_si128_emu(acc, lazyLengthHash_port(1024, 64));
    return precompReduction64_port(acc);
}
